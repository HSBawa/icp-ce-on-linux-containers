    ...
    config {
     ...
     device {
       ...
       properties {
         ...
         # ipv4.address="${cidrhost(element(split(",",var.lxd_network[var.ipv4_cidr]), count.index % var.ipv4_cidr_count), var.proxy_node[var.start_host_num] + (count.index % var.ipv4_cidr_count))}"
         ipv4.address="${cidrhost(element(split(",",var.lxd_network[var.ipv4_cidr]), count.index % var.ipv4_cidr_count), var.proxy_node[var.start_host_num] + count.index)}"
       }
     }

     #### Root device
     device {
       name = "${var.proxy_node[var.storage_device_name]}"
       type = "${var.proxy_node[var.storage_device_type]}"
       properties {
         path="${var.proxy_node[var.storage_device_path]}"
         pool="${var.proxy_node[var.storage_device_pool]}"
         size="${var.proxy_node[var.storage_device_size]}"
       }
     }
}
